...
Then rise a Renewal request with  first RPMUSER in the deadline analyser section with "5-05-2020" to "8-25-2023"
...
Then renewal application send to under review with First legal user
Then renewal application send to FCC  from under review page with first legal user
...
Then rise a renewal request with  second RPMUSER in the deadline analyser section with "5-05-2020" to "8-25-2023"
...
Then rise a renewal request with Third RPMUSER in the deadline analyser section with "5-05-2020" to "8-25-2023"
...
@RPMUSER4
Scenario: Rise a Renewal request with fourth RPMUSER for deadline analyser moduel
Given enter url application into "chrome" for Deadlineanlayser in the "Stage" environment with fourth RPMUSER for Renewal process
When fourth RPMUSER login with valid credtinlas for Deadline analyser Renewal process
Then rise a Renewal request with  fourth RPMUSER in the deadline analyser section with "5-05-2020" to "8-25-2023"
Then close browser after  Renewal Request rise a request with  fourth RPMUSER 

@NOCUser4
Scenario: Check the status of the renwal application with fourth NOCuser for deadline analyser moduel

When fourth NOC user login with valid credtinals  for renewal process for Deadline analyser moduel
Then Check the "approved" status of the Renewal application for deadLine analyser moduel with  fourth NOC user
Then close browser after check the status of the renewal application with  fourth NOC user for deadLine analyser moduel

@LegalUser4
Scenario: Check the status of the cancelation application with fourth Legaluser for deadline analyser moduel

When fourth legal user login with valid credtinals  for Cancelation process for Deadline analyser moduel
Then renewal application send to under review with fourth legal user
Then renewal application reject from under review page with fourth legal user
Then close browser after check the status of the renewal application with  fourth legal user for deadLine analyser moduel
